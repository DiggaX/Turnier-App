...
**Stand:** 2026-08-09 · Branch `main` @ `55d2c32` · **gepusht und live** unter https://turnier-app-opal.vercel.app (Push auf `main` deployt automatisch, siehe §3)
...
7. ⚠️ **`accept_invite` kann unter Race mehrfach eingelöst werden** (2026-08-09 gefunden, ungefixt): die Funktion macht `select` → Prüfung → `insert profiles` → `update org_invites`. Zwei gleichzeitige Einlösungen desselben Codes passieren beide die Prüfung, beide legen ein Profil an. Genau der Fehler, der bei `device_pairings` schon behoben wurde (§6: „Einlösung per **einem** bedingten UPDATE"). Fix: das `update … where code = p_code and accepted_at is null and expires_at > now() returning *` zum Gate machen und erst danach das Profil anlegen; die Vorabprüfung nur noch für die Fehlermeldungen behalten.
8. **Supabase-Advisor** (2026-08-09): 61 Hinweise, **0 ERROR**. Die 45 „SECURITY DEFINER ist aufrufbar"-Warnungen sind bei dieser Architektur erwartbar — die Guards sitzen in den Funktionen, stichprobenartig verifiziert (`set_member_role`/`remove_member`: `is_admin()` + Org + Selbstschutz; `get_scorekeeper_tokens`: `is_staff()` + Org). Die 13 „Anonymous Access"-Warnungen sind Form, nicht Substanz: die Policies stehen auf `public`, ihre `USING`-Klauseln sind für `anon` falsch. Offen und in einem Klick zu erledigen: **Leaked Password Protection ist im Auth-Dashboard aus**.
